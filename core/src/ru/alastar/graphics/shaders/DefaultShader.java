...
public class DefaultShader extends BaseShader {
    ...
    public static String getDefaultVertexShader () {
        if (defaultVertexShader == null)
            defaultVertexShader = Gdx.files.classpath("ru/alastar/graphics/shaders/default_v2.glsl").readString();
    ...
    public static String getDefaultFragmentShader () {
        if (defaultFragmentShader == null)
            defaultFragmentShader = Gdx.files.classpath("ru/alastar/graphics/shaders/default_f2.glsl").readString();
    ...
    protected final int u_Lights0color = register(new Uniform("u_Lights[0].color"));
    protected final int u_Lights0dir = register(new Uniform("u_Lights[0].direction"));
    protected final int u_Lights0position = register(new Uniform("u_Lights[0].position"));

    protected final int u_Lights0projViewTrans = register(new Uniform("u_Lights[0].projView"));
    protected final int u_Lights0depthmap = register(new Uniform("u_Lights[0].depthMap"));

    protected final int u_Lights0PCFOffset = register(new Uniform("u_Lights[0].offset"));
    protected final int u_Lights0intensity = register(new Uniform("u_Lights[0].intensity"));
    protected final int u_Lights0angle = register(new Uniform("u_Lights[0].angle"));

    protected final int u_Lights0type = register(new Uniform("u_Lights[0].type"));

    protected final int u_Lights1color = register(new Uniform("u_Lights[1].color"));
    ...
    protected int LightsColorOffset;
    protected int LightsDirectionOffset;
    protected int LightsPositionOffset;
    protected int LightsProjViewTransOffset;
    protected int LightsDepthmapOffset;
    protected int LightsPCFOffset;
    protected int LightsIntensityOffset;
    protected int LightsAngleOffset;
    ...
    protected final ArrayList<? extends BaseLight> lights;
    ...
    public DefaultShader (final Renderable renderable, final Config config, final ShaderProgram shaderProgram) {
        ...
        this.lights = (((GEnvironment)renderable.environment).getLights());
    ...
    public void init () {
        ...
        LightsLoc = loc(u_Lights0color);
        LightsColorOffset = loc(u_Lights0color) - LightsLoc;
        LightsDirectionOffset = loc(u_Lights0dir) - LightsLoc;
        LightsPositionOffset = loc(u_Lights0position) - LightsLoc;
        LightsProjViewTransOffset = loc(u_Lights0projViewTrans) - LightsLoc;
        LightsDepthmapOffset = loc(u_Lights0depthmap) - LightsLoc;
        LightsPCFOffset = loc(u_Lights0PCFOffset) - LightsLoc;
        LightsIntensityOffset = has(u_Lights0intensity) ? loc(u_Lights0intensity) - LightsLoc : -1;
        LightsAngleOffset = loc(u_Lights0angle) - LightsLoc;
        LightsTypeOffset = loc(u_Lights0type) - LightsLoc;

        LightsSize = loc(u_Lights1color) - LightsLoc;
    ...
    protected void bindLights (final Renderable renderable, final Attributes attributes) {
        ...
        for (int i = 0; i < lights.size(); i++) {
            if(lights.get(i) instanceof  Light) {
                l = (Light)lights.get(i);
                int idx = LightsLoc + idh * LightsSize;

                program.setUniformf(idx + LightsColorOffset, lights.get(i).color.r,
                        lights.get(i).color.g, lights.get(i).color.b);
                program.setUniformf(idx + LightsDirectionOffset, l.direction.x, l.direction.y, l.direction.z);
                program.setUniformf(idx + LightsPositionOffset, l.position.x, l.position.y,
                        l.position.z);

                program.setUniformMatrix(idx + LightsProjViewTransOffset, l.getProjView());

                program.setUniformi(idx + LightsDepthmapOffset, context.textureBinder.bind(l.getDepthMap()));
                ++idh;
                if (LightsSize <= 0) break;
                if (LightsLoc >= 0 && lights.get(i) instanceof GDirectionalLight) {
                    program.setUniformi(idx + LightsTypeOffset, 1);

                } else if (LightsLoc >= 0 && lights.get(i) instanceof GSpotLight) {
                    program.setUniformf(idx + LightsAngleOffset, ((GSpotLight)l).angle);
                    program.setUniformi(idx + LightsTypeOffset, 2);

                } else if (LightsLoc >= 0 && lights.get(i) instanceof GPointLight) {
                    program.setUniformi(idx + LightsTypeOffset, 3);
                }
            }
        }
        if (attributes.has(ColorAttribute.Fog)) {
